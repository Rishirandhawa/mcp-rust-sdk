...
async fn main() -> McpResult<()> {
    ...
    match session.connect(transport).await {
        Ok(init_result) => {
            tracing::info!("Connected to HTTP server: {} v{}", 
...
async fn demonstrate_http_operations(client: &std::sync::Arc<tokio::sync::Mutex<McpClient>>) -> McpResult<()> {
    ...
    tracing::info!("=== Calling HTTP Calculator Tool ===");
    {
        ...
        match client_guard.call_tool("http_calculator".to_string(), Some(args)).await {
            Ok(result) => {
    ...
    {
        let client_guard = client.lock().await;
        match client_guard.read_resource("http://server/status".to_string()).await {
            Ok(result) => {
    ...
    {
        let client_guard = client.lock().await;
        match client_guard.read_resource("http://server/metrics".to_string()).await {
            Ok(result) => {
